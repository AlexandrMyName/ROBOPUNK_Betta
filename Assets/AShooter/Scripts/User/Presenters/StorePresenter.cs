...
namespace User.Presenters
{
    ...
    public class StorePresenter : MonoBehaviour
    {
                            ...
                            UnityAction<int, float> onClickDamageButton)
        {
            ...
            _speedButton.onClick.AddListener(() => onClickSpeedButton(_itemConfigs.SpeedItems.price, ((_itemConfigs.SpeedItems.improvementCoefficient/100)+1)));
            _damageButton.onClick.AddListener(() => onClickDamageButton(_itemConfigs.DamageItems.price, ((_itemConfigs.DamageItems.improvementCoefficient/100)+1)));

            SetInscription(_healthButton, _itemConfigs.HealthItems, "un.");
            SetInscription(_speedButton, _itemConfigs.SpeedItems, "%");
            SetInscription(_damageButton, _itemConfigs.DamageItems, "%");
        }


        private void SetInscription(Button btn, StoreItemConfig items, string dimension)
        {
            var txt = btn.GetComponentInChildren<TMP_Text>();
            txt.text = $"+{items.improvementCoefficient}{dimension} {items.name} -> {items.price}g";
        }
